...
public class AnimationModelImpl implements AnimationModel {
  ...
  @Override
  public void newMotion(String shapeName, int t1, int x1, int y1, int w1,
    ...
    Motion motion = buildMotion(shapeName, t1, x1, y1, w1, h1, r1, g1, b1, t2, x2, y2, w2, h2, r2,
        g2,
        b2);
    ...
    for (int i = 0; i < animationLog.get(shapeName).size(); i++) {
      ...
      if (motion.getStartTick() == m.getStartTick() && motion.getEndTick() == m.getStartTick()) {
...
//      }
  ...
  private Motion buildMotion(String shapeName, int t1, int x1, int y1, int w1,
      int h1, int r1, int g1, int b1, int t2, int x2, int y2, int w2, int h2, int r2, int g2,
      int b2) {
    Position2D startP = new Position2D(x1, y1);
    Position2D endP = new Position2D(x2, y2);
    Size startS = new Size(w1, h1);
    Size endS = new Size(w2, h2);
    ShapeColor startC = new ShapeColor(r1, g1, b1);
    ShapeColor endC = new ShapeColor(r2, g2, b2);
    Shape s = shapes.get(shapeName);
    return new Motion(s.getCorrectShape(shapeName, startP, startS, startC), t1,
        s.getCorrectShape(shapeName, endP, endS, endC), t2);
  }

  public void removeMotionAndAdjust(String shapeName, int t1, int x1, int y1, int w1,
      int h1, int r1, int g1, int b1, int t2, int x2, int y2, int w2, int h2, int r2, int g2,
      int b2) {
    ...
    Motion m = this.buildMotion(shapeName, t1, x1, y1, w1, h1, r1, g1, b1, t2, x2, y2, w2, h2, r2,
        g2, b2);
    if (!this.animationLog.get(shapeName).contains(m)) {
      throw new IllegalArgumentException("This motion doesn't exist");
    }

    for (int i = 0; i < this.animationLog.get(shapeName).size(); i++) {

      if (t1 == this.animationLog.get(shapeName).get(i).getStartTick() &&
          t2 == this.animationLog.get(shapeName).get(i).getEndTick()) {

        if ((i != 0)) {
          int oldEndTick = this.animationLog.get(shapeName).get(i).getEndTick();
          double oldEndX = this.animationLog.get(shapeName).get(i).getEndShape().getPosition()
              .getX();
          double oldEndY = this.animationLog.get(shapeName).get(i).getEndShape().getPosition()
              .getY();
          int oldEndWidth = this.animationLog.get(shapeName).get(i).getEndShape().getSize()
              .getWidth();
          int oldEndHeight = this.animationLog.get(shapeName).get(i).getEndShape().getSize()
              .getHeight();
          int oldEndR = this.animationLog.get(shapeName).get(i).getEndShape().getColor().getRed();
          int oldEndG = this.animationLog.get(shapeName).get(i).getEndShape().getColor().getGreen();
          int oldEndB = this.animationLog.get(shapeName).get(i).getEndShape().getColor().getBlue();

          int oldStartTick = this.animationLog.get(shapeName).get(i - 1).getStartTick();
          double oldStartX = this.animationLog.get(shapeName).get(i - 1).getStartShape()
              .getPosition()
              .getX();
          double oldStartY = this.animationLog.get(shapeName).get(i - 1).getStartShape()
              .getPosition()
              .getY();
          int oldStartWidth = this.animationLog.get(shapeName).get(i - 1).getStartShape().getSize()
              .getWidth();
          int oldStartHeight = this.animationLog.get(shapeName).get(i - 1).getStartShape().getSize()
              .getHeight();
          int oldStartR = this.animationLog.get(shapeName).get(i - 1).getStartShape().getColor()
              .getRed();
          int oldStartG = this.animationLog.get(shapeName).get(i - 1).getStartShape().getColor()
              .getGreen();
          int oldStartB = this.animationLog.get(shapeName).get(i - 1).getStartShape().getColor()
              .getBlue();
          this.animationLog.get(shapeName).remove(i);
          this.animationLog.get(shapeName).remove(i - 1);
          this.newMotion(shapeName, oldStartTick, (int) oldStartX, (int) oldStartY, oldStartWidth,
              oldStartHeight, oldStartR, oldStartG, oldStartB,
              oldEndTick, (int) oldEndX, (int) oldEndY, oldEndWidth,
              oldEndHeight, oldEndR, oldEndG, oldEndB);
        } else {
          this.animationLog.get(shapeName).remove(i);
        }
      }
  ...
  public void removeMotion(String shapeName, int t1, int x1, int y1, int w1,
      int h1, int r1, int g1, int b1, int t2, int x2, int y2, int w2, int h2, int r2, int g2,
      int b2) {
    if (!this.shapes.containsKey(shapeName)) {
      throw new IllegalArgumentException("This shape doesn't exist");
  ...
  public void addShape(String name, String type) {
    List<String> shapeTypes = Arrays.asList("rectangle", "square", "ellipse");
    ...
    Shape s = new ShapeFactory().createShape(type, name, defaultPosition, defaultSize,
        defaultColor);
    shapes.put(name, s);
    animationLog.put(name, new ArrayList<>());
  }
  ...
  public Shape findShapeAtTick(String shapeName, int t) {
    ...
    if (this.animationLog.get(shapeName).isEmpty()) {
      return null;
    }
    ...
    // if shape can't be found at the tick
    return null;
  }
  ...
  public ArrayList<Integer> getKeyFrameTicks(String shapeName) throws IllegalArgumentException {
    ...
    for (int i = 0; i < animationLog.get(shapeName).size(); i++) {
      ticks.add(animationLog.get(shapeName).get(i).getStartTick());
      if (i == animationLog.get(shapeName).size() - 1) {
        ticks.add(animationLog.get(shapeName).get(i).getEndTick());
      }
    }
...
//          defaultColor);
    ...
    public AnimationBuilder<AnimationModel> addMotion(String name, int t1, int x1, int y1, int w1,
      ...
      model.newMotion(name, t1, x1, y1, w1, h1, r1, g1, b1, t2, x2, y2, w2, h2, r2, g2, b2);
      return this;
    }
