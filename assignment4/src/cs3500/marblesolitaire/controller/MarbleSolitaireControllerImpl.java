...
/**
 * Class that implements the MarbleSolitaireController interface which takes in user inputs via a
 * readable and displays the appropriate messages to the console via the appendable based on the
 * model throws an illegal argument exception if any of the inputs are null.
 */
public class MarbleSolitaireControllerImpl implements MarbleSolitaireController {
  ...
  private boolean ifQuit(String input) {
    return input.equals("q") || input.equals("Q");
  }

  private void quit(MarbleSolitaireModel model) {

    appendToConsole("Game quit!\n" + "State of game when quit:" + "\n" + model.getGameState()
  ...
  public void playGame(MarbleSolitaireModel model)
    ...
    while (!(model.isGameOver())) {
      ...
      if (!(isNumber(current) || ifQuit(current))) {
